...
class PackboxList extends StatefulWidget {
  const PackboxList({Key? key}) : super(key: key);

  @override
  _PackboxListState createState() => _PackboxListState();
}

class _PackboxListState extends State<PackboxList> {
  ...
  void initState() {
    ...
    context.read<DoclistBloc>()
      ..add(DoclistLoaded(fromDate: datenow, toDate: datenow, status: status));
  ...
  Widget statusDoc() {
    List<String> _items = ['ทั้งหมด', 'Incomplete', 'Complete'];
    return Container(
      margin: EdgeInsets.only(left: 5, right: 5, top: 10),
      child: BlocBuilder<DoclistBloc, DoclistState>(
        builder: (context, state) {
  ...
  Future<void> scanBarcodeNormal() async {
    ...
    try {
      ...
      context.read<DoclistBloc>()..add(DoclistLoaded(keyWord: docNo.text));
...
Widget docList(BuildContext context, List<DocList> docList, double height) {
  return Container(
                ...
                child: SizedBox(
                  ...
                  child: ListView.builder(
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        return cardDocList(context, docList[index]);
                      }),
                ),
              )
            ],
...
Widget cardDocList(BuildContext context, DocList docList) {
  final handleCardClick = () => Navigator.of(context).push(MaterialPageRoute(
      builder: (_) => PackboxDetail(
            status: docList.status,
            docnoselect: docList.docNo,
          )));

  switch (docList.docListStatus) {
    case DocListStatus.success:
      return DoccardSuccess(context, docList, onTap: handleCardClick);
    case DocListStatus.warning:
      return DoccardWarning(context, docList, onTap: handleCardClick);
    case DocListStatus.unknown:
    default:
      return DoccardShadow(context, docList);
  }
}

Widget DoccardSuccess(BuildContext context, DocList docList,
    {VoidCallback? onTap}) {
  return DoccardDefault(context, docList,
      onTap: onTap,
...
Widget DoccardWarning(BuildContext context, DocList docList,
    {VoidCallback? onTap}) {
  return DoccardDefault(context, docList,
      onTap: onTap,
      shape: RoundedRectangleBorder(
...
Widget DoccardShadow(BuildContext context, DocList docList,
    {VoidCallback? onTap}) {
...
Widget DoccardDefault(
  ...
  List<String> _dateThaiFormat = docList.docDate.split("-");
  ...
  String title = '';
  double item_wait =
      double.parse(docList.itemCount) - double.parse(docList.itemSuccess);
  if (docList.transFlag == "56") {
    title += 'ใบเบิก: ' + docList.docNo;
  } else {
    title += 'ใบขาย: ' + docList.docNo;
  }
  String subText = 'รวมสินค้า: ' + docList.itemCount;

  String subText2 = '     จัดเสร็จ:' + docList.itemSuccess;
            ...
            ElevatedButton(
              ...
              onPressed: () {
                Navigator.of(context).push(MaterialPageRoute(
                    builder: (_) => PackboxDetail(
                        docnoselect: docList.docNo, status: docList.status)));
              },
            ),
            ...
            ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                  ...
                  textStyle:
                      TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
              onPressed: () {
                if (item_wait == 0 && docList.status == 0) {
                  //showAlertSend(context, docList.docNo);
                  print('send success');
                }
              },
            ),
