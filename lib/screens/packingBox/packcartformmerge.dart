...
class CartFormMerge extends StatefulWidget {
  final String? docnoselect;
  final List<StorePack> cartselect;
  final int status;
  const CartFormMerge(
      {Key? key,
      required this.docnoselect,
      required this.status,
      required this.cartselect})
      : super(key: key);

  @override
  _CartFormMergeState createState() => _CartFormMergeState();
}

class _CartFormMergeState extends State<CartFormMerge> {
  ...
  void setUpdateQty() {
    widget.cartselect.forEach((ele) {
      ele.details.forEach((data) {
        context.read<DocdetailBloc>()
          ..add(ProductQtyIncreasedMerge(
        ...
        actions: <Widget>[
          IconButton(
              ...
              onPressed: () {
                ...
                if (storeState is DocDetailLoadSuccess) {
                  int checkError = 0;
                  storeState.docDetail.forEach((element) {
                    if (element.qty_scan > element.qty_wait) {
                      checkError++;
                    }
                  });
                  if (checkError == 0) {
    ...
    return new Card(
        shape: RoundedRectangleBorder(
          side: docdetail.qty_scan == docdetail.qty_wait
              ...
              : (docdetail.qty_scan >= docdetail.qty_wait &&
                      docdetail.isPremium)
                  ? BorderSide(color: Colors.green, width: 2.0)
                  : (docdetail.qty_scan > docdetail.qty_wait &&
                          !docdetail.isPremium)
