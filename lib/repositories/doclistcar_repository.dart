...
class DoclistCarRepository {
  Future<ApiResponse> fetchAllDoclistCar(
      {String search = "",
      String from_date = "",
      String to_date = "",
    ...
    try {
      final response = await client.get('/GetDocShipment', queryParameters: {
  ...
  Future<ApiResponse> fetchAllDoccardetail({String doc_no = ""}) async {
    Dio client = Client().init();
    final appConfig = GetStorage('AppConfig');
    try {
      final response =
          await client.get('/GetDocPackCarDetail', queryParameters: {
        'provider_name': appConfig.read("hoProviderName") ?? AppConfig.provider,
        'database_name':
            appConfig.read("hoDatabaseName") ?? AppConfig.databaseName,
        'doc_no': doc_no,
